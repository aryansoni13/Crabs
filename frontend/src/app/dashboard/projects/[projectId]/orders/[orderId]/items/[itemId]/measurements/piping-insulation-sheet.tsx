...
const FITTING_FIELDS: { key: FittingKey; label: string }[] = [
  { key: "qtyElbow90", label: "90 Elbow" },
  { key: "qtyElbow45", label: "45 Elbow" },
  { key: "qtyTee", label: "Tee" },
  { key: "qtyReducer", label: "Reducer" },
  { key: "qtyEndCap", label: "End Cap" },
  { key: "qtyFlangeRem", label: "Flg Rem" },
  { key: "qtyValveRem", label: "Vlv Rem" },
  { key: "qtyFlangeFix", label: "Flg Fix" },
  { key: "qtyValveFix", label: "Vlv Fix" },
  { key: "qtyWeldValveFix", label: "Weld Vlv" },
];
...
const createEmptyRow = (): MeasurementRow => ({
  ...
  qtyElbow90: undefined,
  qtyElbow45: undefined,
  qtyTee: undefined,
  qtyReducer: undefined,
  qtyEndCap: undefined,
  qtyFlangeRem: undefined,
  qtyValveRem: undefined,
  qtyFlangeFix: undefined,
  qtyValveFix: undefined,
  qtyWeldValveFix: undefined,
...
const withCalculatedValues = (row: MeasurementRow): MeasurementRow => {
  const calculated = calculatePipingInsulationValues({
    lineSize: row.lineSize,
    pipeLength: row.pipeLength,
    pipeOD: row.pipeOD,
    insulationThickness: row.insulationThickness,
    qtyElbow90: row.qtyElbow90,
    qtyElbow45: row.qtyElbow45,
    qtyTee: row.qtyTee,
    qtyReducer: row.qtyReducer,
    qtyEndCap: row.qtyEndCap,
    qtyFlangeRem: row.qtyFlangeRem,
    qtyValveRem: row.qtyValveRem,
    qtyFlangeFix: row.qtyFlangeFix,
    qtyValveFix: row.qtyValveFix,
    qtyWeldValveFix: row.qtyWeldValveFix,
  });
  ...
  return {
    ...
    total: calculated.rmt,
  };
};

const buildPayload = (row: MeasurementRow) => ({
  location_description: row.location || row.drawingNo,
  length: row.pipeLength || 0,
  breadth: row.pipeOD || 0,
  depth: row.insulationThickness || 0,
  ...
  custom_fields: {
    ...
    qtyValveFix: row.qtyValveFix,
    qtyWeldValveFix: row.qtyWeldValveFix,
    totalFittingsLength: row.totalFittingsLength,
    rmt: row.rmt,
    area: row.area,
...
const hydrateRow = (measurement: any): MeasurementRow =>
  withCalculatedValues({
    id: measurement.id,
    location: measurement.custom_fields?.location || measurement.location_description || "",
    drawingNo: measurement.custom_fields?.drawingNo || "",
    sheetNo: measurement.custom_fields?.sheetNo || "",
    moc: measurement.custom_fields?.moc || "",
    lineSize: String(measurement.custom_fields?.lineSize || ""),
    pipeOD: measurement.custom_fields?.pipeOD ?? measurement.breadth ?? undefined,
    insulationThickness:
      measurement.custom_fields?.insulationThickness ?? measurement.depth ?? undefined,
    insulationType: measurement.custom_fields?.insulationType || "",
    temp: measurement.custom_fields?.temp || "",
    pipeLength: measurement.length ?? undefined,
    qtyElbow90: measurement.custom_fields?.qtyElbow90 ?? undefined,
    qtyElbow45: measurement.custom_fields?.qtyElbow45 ?? undefined,
    qtyTee: measurement.custom_fields?.qtyTee ?? undefined,
    qtyReducer: measurement.custom_fields?.qtyReducer ?? undefined,
    qtyEndCap: measurement.custom_fields?.qtyEndCap ?? undefined,
    qtyFlangeRem: measurement.custom_fields?.qtyFlangeRem ?? undefined,
    qtyValveRem: measurement.custom_fields?.qtyValveRem ?? undefined,
    qtyFlangeFix: measurement.custom_fields?.qtyFlangeFix ?? undefined,
    qtyValveFix: measurement.custom_fields?.qtyValveFix ?? undefined,
    qtyWeldValveFix: measurement.custom_fields?.qtyWeldValveFix ?? undefined,
    totalFittingsLength: Number(measurement.custom_fields?.totalFittingsLength || 0),
    rmt: Number(measurement.custom_fields?.rmt || measurement.quantity || 0),
    area: Number(measurement.custom_fields?.area || 0),
...
const hasRequiredValues = (row: MeasurementRow) =>
  Boolean(row.location.trim() || row.drawingNo.trim());

export default function PipingInsulationMeasurementSheet({
  projectId,
  orderId,
  itemId,
}: Props) {
  ...
  useEffect(() => {
    ...
    editRow.pipeLength,
    editRow.pipeOD,
    editRow.insulationThickness,
    editRow.qtyElbow90,
    editRow.qtyElbow45,
    editRow.qtyTee,
    editRow.qtyReducer,
    editRow.qtyEndCap,
    editRow.qtyFlangeRem,
    editRow.qtyValveRem,
    editRow.qtyFlangeFix,
    editRow.qtyValveFix,
    editRow.qtyWeldValveFix,
    editingRowId,
  ]);
  ...
  const filteredMeasurements = measurements.filter((measurement) => {
    const values = {
      location: String(
        measurement.custom_fields?.location || measurement.location_description || "",
      ).toLowerCase(),
    ...
    if (searchQuery) {
      ...
      const matchesQuery =
        Object.values(values).some((value) => value.includes(query)) ||
        FITTING_FIELDS.some((field) =>
          String(measurement.custom_fields?.[field.key] || "")
            .toLowerCase()
            .includes(query),
        );
  ...
  const renderEditableCells = (
    ...
    <>
      <td className="px-4 py-2"><input autoFocus={autoFocusFirst} value={row.location} onChange={(e) => updater({ location: e.target.value })} className={inputClass} placeholder="Location" /></td>
      ...
      <td className="px-4 py-2"><input value={row.lineSize} onChange={(e) => updater({ lineSize: e.target.value })} className={inputClass} placeholder="Line Size" /></td>
      <td className="px-4 py-2"><input type="number" step="0.001" value={row.pipeOD ?? ""} onChange={(e) => updater({ pipeOD: e.target.value ? Number(e.target.value) : undefined })} className={inputNumClass} placeholder="0" /></td>
      <td className="px-4 py-2"><input type="number" step="0.001" value={row.insulationThickness ?? ""} onChange={(e) => updater({ insulationThickness: e.target.value ? Number(e.target.value) : undefined })} className={inputNumClass} placeholder="0" /></td>
      ...
      <td className="px-4 py-2"><input value={row.temp} onChange={(e) => updater({ temp: e.target.value })} className={inputClass} placeholder="Temp" /></td>
      <td className="px-4 py-2"><input type="number" step="0.001" value={row.pipeLength ?? ""} onChange={(e) => updater({ pipeLength: e.target.value ? Number(e.target.value) : undefined })} className={inputNumClass} placeholder="0" /></td>
      {FITTING_FIELDS.map((field) => (
        <td key={field.key} className="px-4 py-2"><input type="number" step="0.001" value={row[field.key] ?? ""} onChange={(e) => updater({ [field.key]: e.target.value ? Number(e.target.value) : undefined } as Partial<MeasurementRow>)} className={inputNumClass} placeholder="0" /></td>
      ))}
      <td className="px-4 py-2"><div className="px-2.5 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded font-mono text-sm text-right font-bold">{Number(row.totalFittingsLength || 0).toFixed(2)}</div></td>
      <td className="px-4 py-2"><div className="px-2.5 py-1.5 bg-slate-900 dark:bg-black text-white rounded font-mono text-sm text-right font-bold">{Number(row.rmt || 0).toFixed(2)}</div></td>
      <td className="px-4 py-2"><div className="px-2.5 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded font-mono text-sm text-right font-bold">{Number(row.area || 0).toFixed(3)}</div></td>
    ...
    <div className="max-w-full mx-auto py-6 px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4 mb-6">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="px-2.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-black uppercase tracking-widest rounded-md">Piping Insulation</span>
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm mb-6">
        ...
        <div className="px-6 py-3 border-b border-slate-100 dark:border-slate-800">
          <div className="relative min-w-[220px]"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" /><input type="text" placeholder="Search by location, drawing, line size..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg focus:border-blue-500 focus:outline-none text-sm font-semibold text-slate-900 dark:text-white placeholder:text-slate-400" /></div>
        </div>
          ...
          <table className="w-full text-left border-collapse text-sm [&_th]:border [&_th]:border-slate-200 dark:[&_th]:border-slate-700 [&_td]:border [&_td]:border-slate-200 dark:[&_td]:border-slate-700">
            <thead className="sticky top-0 z-10 shadow-sm">
              <tr className="bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                ...
                <th className="px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider min-w-[140px]">Location</th>
                ...
                <th className="px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider min-w-[100px]">Line Size</th>
                <th className="px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right min-w-[110px]">Pipe OD</th>
                <th className="px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right min-w-[120px]">Ins. Thick</th>
                <th className="px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider min-w-[140px]">Ins. Type</th>
                <th className="px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider min-w-[90px]">Temp</th>
                <th className="px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right min-w-[110px]">Pipe Length</th>
                {FITTING_FIELDS.map((field) => <th key={field.key} className="px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right min-w-[95px]">{field.label}</th>)}
                <th className="px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right min-w-[120px]">Fittings Len</th>
                <th className="px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right min-w-[100px]">RMT</th>
                <th className="px-4 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right min-w-[100px]">Area</th>
              ...
              <tr className="border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
                ...
                <td className="px-4 py-2"><input value={columnFilters.location} onChange={(e) => setColumnFilters((prev) => ({ ...prev, location: e.target.value }))} className={inputClass} placeholder="Search..." /></td>
                ...
                <td className="px-4 py-2"></td><td className="px-4 py-2"></td>
                {FITTING_FIELDS.map((field) => <td key={field.key} className="px-4 py-2"></td>)}
            ...
            <tbody className="divide-y divide-slate-100/50 dark:divide-slate-800/50">
              {filteredMeasurements.length === 0 && !newRowActive && (
                <tr><td colSpan={25 + departmentMilestones.length} className="px-6 py-12 text-center"><p className="text-sm text-slate-400 font-semibold">No measurement entries match your filters. <button onClick={handleAddRow} className="text-blue-600 hover:underline font-bold">Add a new row</button> to get started.</p></td></tr>
              )}
              {filteredMeasurements.map((measurement, index) => {
                ...
                return (
                  <tr key={measurement.id} className="hover:bg-blue-50/30 dark:hover:bg-blue-900/5 transition-colors group">
                    ...
                    <td className="px-4 py-3 text-sm font-semibold text-slate-900 dark:text-white">{measurement.custom_fields?.location || measurement.location_description || "-"}</td>
                    ...
                    <td className="px-4 py-3 text-xs font-semibold text-slate-600 dark:text-slate-400">{measurement.custom_fields?.lineSize || "-"}</td>
                    <td className="px-4 py-3 text-xs font-mono text-right text-slate-600 dark:text-slate-400">{measurement.custom_fields?.pipeOD ?? "-"}</td>
                    <td className="px-4 py-3 text-xs font-mono text-right text-slate-600 dark:text-slate-400">{measurement.custom_fields?.insulationThickness ?? "-"}</td>
                    ...
                    <td className="px-4 py-3 text-xs font-semibold text-slate-600 dark:text-slate-400">{measurement.custom_fields?.temp || "-"}</td>
                    <td className="px-4 py-3 text-xs font-mono text-right text-slate-600 dark:text-slate-400">{measurement.length ?? "-"}</td>
                    {FITTING_FIELDS.map((field) => <td key={field.key} className="px-4 py-3 text-xs font-mono text-right text-slate-600 dark:text-slate-400">{measurement.custom_fields?.[field.key] ?? "-"}</td>)}
                    <td className="px-4 py-3 text-sm font-bold font-mono text-right text-slate-700 dark:text-slate-300">{Number(measurement.custom_fields?.totalFittingsLength || 0).toFixed(2)}</td>
                    <td className="px-4 py-3 text-sm font-bold font-mono text-right text-blue-600 dark:text-blue-400">{Number(measurement.custom_fields?.rmt || measurement.quantity || 0).toFixed(2)}</td>
                    <td className="px-4 py-3 text-sm font-bold font-mono text-right text-slate-700 dark:text-slate-300">{Number(measurement.custom_fields?.area || 0).toFixed(3)}</td>
            ...
            <tfoot><tr className="bg-slate-50 dark:bg-slate-800/30 border-t-2 border-slate-200 dark:border-slate-700 font-bold"><td colSpan={24} className="px-4 py-3 text-sm font-extrabold text-slate-900 dark:text-white">Subtotal</td><td className="px-4 py-3 text-sm font-black text-right text-blue-600 dark:text-blue-400 font-mono">{filteredTotal.toFixed(2)} {item?.unit || "Unit"}</td>{departmentMilestones.map((milestone, index) => { const total = filteredMeasurements.reduce((sum, measurement) => sum + Number(measurement.custom_fields?.milestone_values?.[milestone.mapKey] || 0), 0); return <td key={index} className="px-4 py-3 text-xs font-bold font-mono text-right text-slate-600 dark:text-slate-400">{total > 0 ? total.toFixed(3) : "-"}</td>; })}<td className="px-4 py-3"></td></tr></tfoot>
          </table>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex items-center gap-5"><div className="w-12 h-12 rounded-xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center text-blue-600"><Weight className="w-6 h-6" /></div><div><div className="flex items-baseline gap-1.5"><span className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">{item?.unit || "Unit"}</span></div><h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none mt-1">{totalMeasured.toFixed(2)}</h3><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Total Measured Quantity</p></div></div>
