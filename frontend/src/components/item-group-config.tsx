...
export default function ItemGroupConfig({
  ...
  const toggleItem = (group: ItemGroup, itemId: string) => {
    ...
    void run(
      () => onUpdateGroup(group.id, { selected_item_ids: selectedItemIds }),
      group.id,
    );
  };
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {groups.map((group) => (
                <div
                  key={group.id}
                  className="overflow-hidden rounded-xl border border-blue-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"
                >
                  <div className="mb-3 flex items-center gap-3">
                    ...
                    <input
                      value={group.name}
                      onChange={(event) =>
                        patchLocalGroup({ ...group, name: event.target.value })
                      }
                      onBlur={(event) =>
                        void run(
                          () =>
                    ...
                    <button
                      onClick={() =>
                        void run(() => onDeleteGroup(group.id), group.id)
                      }
                  ...
                  <div className="border-t border-slate-200 px-4 pb-4 pt-3 dark:border-slate-800">
                    <p className="mb-3 text-xs font-black text-slate-500">
                      Select items for this group
                    </p>
                    <div className="grid gap-2 sm:grid-cols-2">
                      {items.map((item) => (
                        <label
                          key={item.id}
                          className="flex cursor-pointer items-start gap-2 rounded-md border border-slate-200 bg-slate-50 p-2 text-xs font-bold text-slate-700 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200"
                        >
                          <input
                            type="checkbox"
